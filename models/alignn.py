...
class LayerNormMLP(nn.Module):
    """Linear feature embedding with sample-local normalization."""
...
class LowRankTypeAdapter(nn.Module):
    """Small zero-initialized adapters around a shared feature transform."""

    def __init__(self, num_types, channels, rank=4):
        super().__init__()
        rank = max(1, min(int(rank), int(channels)))
        self.down = nn.Parameter(torch.empty(num_types, channels, rank))
        self.up = nn.Parameter(torch.zeros(num_types, rank, channels))
        nn.init.xavier_uniform_(self.down)

    def forward(self, x, type_id):
        if x.size(0) == 0:
            return x
        return (x @ self.down[int(type_id)]) @ self.up[int(type_id)]


class SharedTypedRelationConv(nn.Module):
    """Shared gated messages with lightweight relation conditioning."""

    def __init__(self, channels, edge_dim, num_relations, adapter_rank=4):
        super().__init__()
        self.channels = channels
        self.src_gate = nn.Linear(channels, channels)
        self.dst_gate = nn.Linear(channels, channels)
        self.edge_gate = nn.Linear(edge_dim, channels)
        self.message_update = nn.Linear(channels, channels)
        self.relation_bias = nn.Embedding(num_relations, channels)
        nn.init.zeros_(self.relation_bias.weight)
        self.relation_adapter = LowRankTypeAdapter(
            num_relations,
            channels,
            rank=adapter_rank,
        )
    ...
    def forward(self, x, edge_index, edge_attr, relation_id):
        ...
        if x_src.size(0) == 0 or x_dst.size(0) == 0 or edge_index.size(1) == 0:
            return message_sum, gate_sum, presence, edge_attr

        src, dst = edge_index
        relation_bias = self.relation_bias.weight[int(relation_id)].view(1, -1)
        edge_update = (
            self.src_gate(x_src[src])
            + self.dst_gate(x_dst[dst])
            + self.edge_gate(edge_attr)
        ...
        sigma = torch.sigmoid(edge_update)
        source_message = (
            self.message_update(x_src)
            + self.relation_adapter(x_src, relation_id)
        )
        message_sum.index_add_(0, dst, source_message[src] * sigma)
        gate_sum.index_add_(0, dst, sigma)
        presence[dst.unique()] = True

        edge_update = F.silu(self.edge_norm(edge_update))
        if edge_attr.size(-1) == self.channels:
            edge_update = edge_attr + edge_update
        return message_sum, gate_sum, presence, edge_update


class SharedTypedNodeUpdate(nn.Module):
    """Shared node update with small atom/defect adapters and LayerNorm."""

    def __init__(self, channels, num_node_types=2, adapter_rank=4):
        super().__init__()
        self.self_update = nn.Linear(channels, channels)
        self.type_bias = nn.Embedding(num_node_types, channels)
        nn.init.zeros_(self.type_bias.weight)
        self.type_adapter = LowRankTypeAdapter(
            num_node_types,
            channels,
            rank=adapter_rank,
        )
    ...
    def forward(self, x, incoming, node_type_id, update_mask=None):
        if x.size(0) == 0:
            return x
        update = (
            self.self_update(x)
            + self.type_adapter(x, node_type_id)
            + self.type_bias.weight[int(node_type_id)].view(1, -1)
            + incoming
        )
        updated = x + F.silu(self.norm(update))
        if update_mask is None:
            return updated
        return torch.where(update_mask.to(dtype=torch.bool), updated, x)


class BidirectionalHeteroUpdate(nn.Module):
    """Host-host, host-to-defect, then defect-to-host staged update."""

    RELATION_NAMES = ("aa", "dd", "ad", "da")
    NODE_TYPE_IDS = {"atom": 0, "defect": 1}

    def __init__(self, hidden_dim, metadata, adapter_rank=4):
        super().__init__()
        self.edge_types = tuple(tuple(edge_type) for edge_type in metadata[1])
        self.edge_type_by_relation = {
            edge_type[1]: edge_type
            for edge_type in self.edge_types
        }
        missing = set(self.RELATION_NAMES) - set(self.edge_type_by_relation)
        if missing:
            raise ValueError(
                f"Bidirectional hetero update is missing relations: {sorted(missing)}"
            )
        self.relation_ids = {
            relation: idx
            for idx, relation in enumerate(self.RELATION_NAMES)
        }
        self.relation_conv = SharedTypedRelationConv(
            hidden_dim,
            hidden_dim,
            num_relations=len(self.RELATION_NAMES),
            adapter_rank=adapter_rank,
        )
        self.node_update = SharedTypedNodeUpdate(
            hidden_dim,
            num_node_types=len(self.NODE_TYPE_IDS),
            adapter_rank=adapter_rank,
        )
        self.relation_logits = nn.Parameter(
            torch.zeros(len(self.RELATION_NAMES))
        )
    ...
    def _relation_message(
        ...
        edge_type = self.edge_type_by_relation[relation]
        message_sum, gate_sum, presence, edge_update = self.relation_conv(
            (x_src, x_dst),
            edge_index_dict[edge_type],
            edge_attr_dict[edge_type],
            self.relation_ids[relation],
        )
        incoming = message_sum / (gate_sum + 1e-6)
        return incoming, presence, edge_type, edge_update

    def _fuse_relations(self, relation_outputs):
        reference = relation_outputs[0][1][0]
        fused = reference.new_zeros(reference.shape)
        weight_sum = reference.new_zeros((reference.size(0), 1))
        any_present = torch.zeros(
            (reference.size(0), 1),
            dtype=torch.bool,
            device=reference.device,
        )
        relation_weights = F.softplus(self.relation_logits)
        for relation, (incoming, presence, _, _) in relation_outputs:
            weight = relation_weights[self.relation_ids[relation]]
            present_float = presence.to(dtype=incoming.dtype)
            fused = fused + weight * incoming * present_float
            weight_sum = weight_sum + weight * present_float
            any_present = any_present | presence
        return fused / weight_sum.clamp_min(1e-6), any_present

    def forward(self, x_dict, edge_index_dict, edge_attr_dict):
        out_edge_attr = dict(edge_attr_dict)

        aa_output = self._relation_message(
            "aa",
            x_dict["atom"],
            x_dict["atom"],
            edge_index_dict,
        ...
        atom_incoming, atom_presence = self._fuse_relations([
            ("aa", aa_output),
        ])
        atom_fea = self.node_update(
            x_dict["atom"],
            atom_incoming,
            self.NODE_TYPE_IDS["atom"],
            update_mask=atom_presence,
        )
        out_edge_attr[aa_output[2]] = aa_output[3]

        dd_output = self._relation_message(
            "dd",
            x_dict["defect"],
            x_dict["defect"],
            edge_index_dict,
        ...
        ad_output = self._relation_message(
            "ad",
            atom_fea,
            x_dict["defect"],
            edge_index_dict,
        ...
        defect_incoming, defect_presence = self._fuse_relations([
            ("dd", dd_output),
            ("ad", ad_output),
        ])
        defect_fea = self.node_update(
            x_dict["defect"],
            defect_incoming,
            self.NODE_TYPE_IDS["defect"],
            update_mask=defect_presence,
        )
        out_edge_attr[dd_output[2]] = dd_output[3]
        out_edge_attr[ad_output[2]] = ad_output[3]

        da_output = self._relation_message(
            "da",
            defect_fea,
            atom_fea,
            edge_index_dict,
        ...
        broadcast_incoming, broadcast_presence = self._fuse_relations([
            ("da", da_output),
        ])
        atom_fea = self.node_update(
            atom_fea,
            broadcast_incoming,
            self.NODE_TYPE_IDS["atom"],
            update_mask=broadcast_presence,
        )
        out_edge_attr[da_output[2]] = da_output[3]
...
class BidirectionalHeteroALIGNNLayer(nn.Module):
    """Shared angle update followed by staged heterogeneous atom updates."""
    ...
    def __init__(
        ...
        self.typed_update = BidirectionalHeteroUpdate(
            hidden_dim,
            metadata,
            adapter_rank=adapter_rank,
        )
    ...
    def forward(self, x_dict, edge_index_dict, edge_attr_dict,
        ...
        x_dict, edge_attr_dict = self.typed_update(
            x_dict,
            edge_index_dict,
            angle_edge_attr_dict,
        )
        return x_dict, edge_attr_dict, angle_attr


class BidirectionalHeteroGraphConvLayer(nn.Module):
    """Post-ALIGNN staged heterogeneous update with the same shared trunk."""
...
class BidirectionalHeteroALIGNN(nn.Module):
    """Parameter-efficient defect-centric bidirectional HeteroALIGNN.

    Geometry encoders and message trunks are shared across node/edge types.
    Lightweight low-rank adapters condition the shared operations, while each
    block explicitly performs host-host, host-to-defect, and updated
    defect-to-host communication.
    """
    ...
    def __init__(
        ...
        self.edge_types = tuple(tuple(edge_type) for edge_type in metadata[1])
        self.hidden_dim = hidden_dim
        self.node_embedding = LayerNormMLP(node_input_shape, hidden_dim)
        self.node_type_embedding = nn.Embedding(
            len(self.NODE_TYPE_IDS),
            hidden_dim,
        )
        ...
        self.edge_embedding = LayerNormMLP(edge_input_shape, hidden_dim)
        self.angle_expansion = AngleExpansion(angle_embed_size)
        self.angle_embedding = LayerNormMLP(
            self.angle_expansion.out_features,
            hidden_dim,
        )
        self.layers = nn.ModuleList([
            BidirectionalHeteroALIGNNLayer(
                hidden_dim,
                metadata,
                adapter_rank=relation_adapter_rank,
                vertex_aggregation=vertex_aggregation,
            )
            for _ in range(n_blocks)
        ])
        self.gcn_layers = nn.ModuleList([
            BidirectionalHeteroGraphConvLayer(
                hidden_dim,
                metadata,
                adapter_rank=relation_adapter_rank,
            )
    ...
    def _line_graph_inputs(self, edge_index_dict, edge_attr_dict, edge_vec_dict):
        ...
        if offset == 0:
            line_edge_index = _empty_index(reference)
            angle_attr = reference.new_empty(
    ...
    def forward(self, x_dict, edge_index_dict, edge_attr_dict, batch_dict,
        ...
        x_dict = {
            node_type: (
                self.node_embedding(x_dict[node_type].float())
                + self.node_type_embedding.weight[
                    self.NODE_TYPE_IDS[node_type]
                ].view(1, -1)
            )
            for node_type in self.node_types
        }
        ...
        reference = next(
            value for value in x_dict.values()
