...
Hidden slides stay in the deck but are skipped during a slide show.
...
```ts
const slide = presentation.slides.add({ background: { fill: fillConfig } });
...
```ts
slide.speakerNotes.text = notesText;
